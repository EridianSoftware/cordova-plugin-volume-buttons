...
public class VolumeButtonsListener extends CordovaPlugin implements OnKeyListener {
   ...
   *
   * @return "boolean" which indicates if the action is valid (true) or not
   *         (false)
   *
   * @date 10/12/2016
   * @version 0.0.3
  ...
  /**
   * Overwritten method for Android application lifecycle. It stops the key events
   * listening process
   *
   * @date 10/12/2016
   * @version 0.0.3
   ...
   *
   * @return "boolean" which indicates if the listener has consumed the event
  ...
  public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
    ...
    if (keyEvent.getAction() == KeyEvent.ACTION_DOWN) {
      ...
      if (keyCode == KeyEvent.KEYCODE_VOLUME_UP && keyEvent.isLongPress()) {

        // Create a new JSONObject with the information and send it
        JSONObject info = new JSONObject();
        try {
          info.put("signal", new String("volume-up press"));
          sendSignal(info, true);
          return true;
        } catch (JSONException ex) {
          Log.e(VolumeButtonsListener_LOG, ex.getMessage());
        }
      } else if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN && keyEvent.isLongPress()) {

        // Create a new JSONObject with the information and send it
        JSONObject info = new JSONObject();
        try {
          info.put("signal", new String("volume-down press"));
          sendSignal(info, true);
          return true;
        } catch (JSONException ex) {
          Log.e(VolumeButtonsListener_LOG, ex.getMessage());
      ...
      JSONObject info = new JSONObject();
      try {
        info.put("signal", new String("volume release"));
        sendSignal(info, true);
        return true;
      } catch (JSONException ex) {
        Log.e(VolumeButtonsListener_LOG, ex.getMessage());
